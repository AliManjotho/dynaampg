...
def plot():
    ...
    iscx_tor_distribution = [2645, 497, 485, 1026, 1529, 1663, 4524, 2139]
    iscx_tor_labels = [
    ...
    sns.barplot(x='Labels', y='Count', data=iscx_vpn_df, ax=ax1, 
                palette=[tuple(c) for c in iscx_vpn_df['Colors']])
    ...
    sns.barplot(x='Labels', y='Count', data=vnat_df, ax=ax2, 
                palette=[tuple(c) for c in vnat_df['Colors']])
    ...
    sns.barplot(x='Labels', y='Count', data=iscx_tor_df, ax=ax3, 
                palette=[tuple(c) for c in iscx_tor_df['Colors']])
    ...
    plot_dataset_distribution(ax4, dataset_name='ISCX-VPN', dataset_labels=iscx_vpn_labels, 
                            file_name='visualization/fig_iscx_distribution_pca.png', method='t-SNE', 
                            num_ood_samples=100, dist_thres=2, color_dict=color_dict)
    plot_dataset_distribution(ax5, dataset_name='VNAT', dataset_labels=vnat_labels, 
                            file_name='visualization/fig_iscx_distribution_tsne.png', method='t-SNE', 
                            num_ood_samples=100, dist_thres=5, color_dict=color_dict)
    plot_dataset_distribution(ax6, dataset_name='ISCX-TOR', dataset_labels=iscx_tor_labels, 
                            file_name='visualization/fig_iscx_distribution_tsne.png', method='t-SNE', 
                            num_ood_samples=100, dist_thres=5, color_dict=color_dict)

    plt.tight_layout(pad=2.0)
    plt.savefig('visualization/fig_plot.png')
    plt.show()


def plot_dataset_distribution(axis, dataset_name, dataset_labels, file_name, method='PCA', num_ood_samples=5, dist_thres=0.8, color_dict=None):
    ...
    legend_elements = list(legend_elements) + [plt.scatter([], [], c='darkred', marker='x', s=50)]
    axis.legend(legend_elements, dataset_labels + ['OOD sample'])
    


    # plt.legend(scatter.legend_elements()[0], dataset_labels + ['OOD Samples'])
    axis.set_title(f'{dataset_name} Feature Distribution ({method})')
